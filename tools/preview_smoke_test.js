...
const report = { base: BASE, startedAt: new Date().toISOString(), checks: [], consoleErrors: [], pageErrors: [] };
...
async function guestToLobby(page, name, username) {
  ...
  await page.waitForSelector('#commanderSelect option', { timeout: 30000 });
...
async function main() {
  ...
  try {
    ...
    // Shared display uses the app's public display query form.
    ...
    await display.waitForFunction(() => /CONNECTED|ONLINE/.test((document.querySelector('#sharedDisplayNet')?.textContent || document.querySelector('#net')?.textContent || '').toUpperCase()), null, { timeout: 25000 }).catch(()=>{});
    await display.waitForFunction(() => (document.querySelector('#sharedDisplayPlayers')?.textContent || '').includes('Smoke Host'), null, { timeout: 20000 });
    ...
    // Read-only invariant: gameplay action controls must not be available in display mode.
    ...
    // Basic deployed asset/runtime integrity.
    ...
    // Ignore expected browser/network noise only after recording it; page errors are always fatal.
